...
import { _cmd } from '../cmd/cmd'
import { _CHANGE_KEY_MAP } from '../event/event'

// create KEdit class
export function KEdit(options) {
  this.init(options)
}
_extend(KEdit, KWidget, {
  init: function (options) {
    ...
    var themesPath = _undef(options.themesPath, ''),
      ...
      isDocumentDomain =
        location.protocol != 'res:' && location.host.replace(/:\d+/, '') !== document.domain,
      srcScript =
        'document.open();' +
      ...
      iframeSrc = _IE
        ...
        : ''
    self.iframe = K(
      '<iframe class="ke-edit-iframe" hidefocus="true" frameborder="0"' + iframeSrc + '></iframe>'
    ).css('width', '100%')
    self.textarea = K('<textarea class="ke-edit-textarea" hidefocus="true"></textarea>').css(
      'width',
      '100%'
    )
