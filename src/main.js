...
function _removeBookmarkTag(html) {
  return _trim(
    html.replace(/<span [^>]*id="?__kindeditor_bookmark_\w+_\d+__"?[^>]*><\/span>/gi, '')
  )
}
...
export function _create(expr, options) {
  ...
  _loadScript(
    editor.langPath + editor.langType + '.js?ver=' + encodeURIComponent(K.DEBUG ? _TIME : _VERSION),
    function () {
      create(editor)
    }
  )
...
_plugin('core', function (K) {
  ...
  self.beforeSetHtml(function (html) {
    ...
    return html
      .replace(/<embed[^>]*type="([^"]+)"[^>]*>(?:<\/embed>)?/gi, function (full) {
        var attrs = _getAttrList(full)
